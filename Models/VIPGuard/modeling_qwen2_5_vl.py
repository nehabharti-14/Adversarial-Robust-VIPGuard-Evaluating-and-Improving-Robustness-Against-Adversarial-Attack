...
from .configuration_qwen2_5_vl import Qwen2_5_VLConfig, Qwen2_5_VLVisionConfig, Qwen2_5_FCConfig

from .FaceChecker import FaceChecker
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, return_hidden=False) -> torch.Tensor:
        """
        Args:
        ...
        proj_hidden_states = self.merger(hidden_states)
        reverse_indices = torch.argsort(window_index)
        proj_hidden_states = proj_hidden_states[reverse_indices, :]
        if return_hidden:
            return proj_hidden_states, hidden_states
        else:
            return proj_hidden_states


class Qwen2_5_FaceCheckerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    config_class = Qwen2_5_FCConfig
    _no_split_modules = []

    def __init__(self, config, *inputs, **kwargs) -> None:
        super().__init__(config, *inputs, **kwargs)
        self.face_checker = FaceChecker(
    ...
    def forward(self, img_feature_q, img_feature_v) -> torch.Tensor:
        """
        Args:
            hidden_states (`torch.Tensor` of shape `(seq_len, hidden_size)`):
                The final hidden states of the model.
        ...
        Returns:
            `torch.Tensor`: hidden_states.
        """
        hidden_states = self.face_checker(img_feature_q, img_feature_v)
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        config.facechecker_config.in_dim = config.vision_config.out_hidden_size
        config.facechecker_config.out_dim = config.vision_config.out_hidden_size
        self.facechecker = Qwen2_5_FaceCheckerPretrainedModel._from_config(config.facechecker_config)
    ...
    def reset_fcchecker(self, facechecker_config):
        self.facechecker = Qwen2_5_FaceCheckerPretrainedModel._from_config(facechecker_config)
        print("The facechecker has been re-initialized.")
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                # Get the feature after projection (image_embeds) and the feature before projection (hidden_features)
                # image_embeds, hidden_feature = self.visual(pixel_values, grid_thw=image_grid_thw, return_hidden=True)
                image_embeds = self.visual(pixel_values, grid_thw=image_grid_thw, return_hidden=False)
                ...
                # NOTE: Process the Face information (when <|face_pad|> > 0)
                if hasattr(self.config, 'face_token_id'):
                    n_face_tokens = (input_ids == self.config.face_token_id).sum().item()
                    merge_size = self.visual.spatial_merge_size
                    if n_face_tokens > 0:
                        if len(image_grid_thw) > 2 or len(image_grid_thw) <=0:
                            AssertionError("Only support when the number of input images is 1 or 2.")

                        if len(image_grid_thw) == 1:
                            v = image_embeds
                            if v.ndim == 3:
                                q = self.facechecker.prompt
                                q = q.unsqueeze(0).expand(v.shape[0], -1, -1)
                            else:
                                q = self.facechecker.face_checker.vip_prompt
                        else:
                            if image_embeds.ndim == 2:
                                first_img_length = (image_grid_thw[0][1] * image_grid_thw[0][2]) // (merge_size ** 2)
                                second_img_length = (image_grid_thw[1][1] * image_grid_thw[1][2]) // (merge_size ** 2)
                        ...
                        if n_face_tokens == 2*n_face_features:
                            face_embeds = torch.concat([q, face_embeds], dim=0)
                        elif n_face_tokens != n_face_features:
                            raise ValueError(
                                f"Image features and image tokens do not match: tokens: {n_face_tokens}, features {n_face_features}"
                            )

                        mask = input_ids == self.config.face_token_id
                        mask_unsqueezed = mask.unsqueeze(-1)
                        mask_expanded = mask_unsqueezed.expand_as(inputs_embeds)
                        face_mask = mask_expanded.to(inputs_embeds.device)

                        face_embeds = face_embeds.to(inputs_embeds.device, inputs_embeds.dtype)
                        inputs_embeds = inputs_embeds.masked_scatter(face_mask, face_embeds)
            ...
            # NOTE: 如果input_embeds不为None, 替换另一种方式处理face token
            # NOTE: Process the Face information (when <|face_pad|> > 0)
            if hasattr(self.config, 'face_token_id'):
                ...
                if n_face_tokens > 0:
                    ...
                    if len(image_grid_thw) == 1:
                        ...
                        first_img_length = (image_grid_thw[0][1] * image_grid_thw[0][2]) // (merge_size ** 2)
                        second_img_length = (image_grid_thw[1][1] * image_grid_thw[1][2]) // (merge_size ** 2)
                        assert first_img_length + second_img_length == image_embeds.shape[0], AssertionError(
                            f"The length of first image is {first_img_length}, The second is {second_img_length}. But the total length is {image_embeds.shape[0]}")
                        v = image_embeds[:first_img_length, :]
                        q = image_embeds[first_img_length:, :]
...
__all__ = ["Qwen2_5_VLForConditionalGeneration", "Qwen2_5_VLModel", "Qwen2_5_VLPreTrainedModel", "Qwen2_5_FaceCheckerPretrainedModel"]
